    ...
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.row}>
        <View style={styles.leftColumn}>
          <Text style={[styles.text, styles.boldText]}>{receiptData.seller.companyName}</Text>
        ...
        <View style={styles.rightColumn}>
          ...
          {receiptData.billTo.line1 === '' ? (
            <>
              ...
              <Text style={styles.text}>{receiptData.billTo.companyName}</Text>
      ...
      <View style={styles.totalRow}>
        <Text style={styles.totalText}>{`Subtotal: $${receiptData.items[0].unitPrice}`}</Text>
      </View>
      <View style={styles.totalRow}>
        <Text style={styles.totalText}>{`Total: $${receiptData.items[0].amount}`}</Text>
      </View>
    </Page>
  </Document>
)
...
const InvoicePdf = ({ orderDetails }) => {
  const receiptData = {
    ...
    datePaid: new Date(orderDetails.orderDateTime).toDateString(),
    seller: {
      companyName: 'Neonflake Enterprises OPC Pvt',
    ...
    billTo: {
      ...
      line1: orderDetails.address.line1,
      line2: orderDetails.address.line2,
      city: orderDetails.address.city,
      state: orderDetails.address.state,
      country: orderDetails.address.country,
      postal_code: orderDetails.address.postal_code,
      email: orderDetails.billingEmail,
    },
    items: [
      {
        description: 'rotuto ai agents',
        qty: 1,
        unitPrice: `${orderDetails.amount}`,
  ...
  return (
    ...
    >
      {({ blob, url, loading, error }) =>
        loading ? 'Generating PDF...' : <CButton color="primary">Download Invoice</CButton>
      }
    </PDFDownloadLink>
  )
}
